...
if [ -f "test_llm_model.py" ]; then
    echo "🧪 Запускаем тест модели (может занять до 3-4 минут на Steam Deck)..."
    if timeout 300 python3 test_llm_model.py; then
        echo "✅ LLM модель работает корректно"
    else
        echo "❌ Проблемы с LLM моделью или таймаут"
        echo "💡 Если модель медленная, попробуйте: ollama pull llama3.2:1b"
        echo "💡 Или запустите тест вручную: python3 test_llm_model.py"
    fi
else
