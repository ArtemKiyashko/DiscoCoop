...
class LLMAgent:
    ...
    async def process_command(self, user_command: str, screenshot: Optional[Image.Image] = None) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            # Логируем хеш скриншота для отслеживания кеширования
            if screenshot:
                import hashlib
                screenshot_hash = hashlib.md5(screenshot.tobytes()).hexdigest()[:8]
                print(f"🖼️  LLM получил скриншот с хешем: {screenshot_hash}")
    ...
    def _build_command_prompt(self, user_command: str, screenshot: Optional[Image.Image] = None) -> str:
        ...
        # Добавляем случайное число для избежания кеширования ответов
        import random
